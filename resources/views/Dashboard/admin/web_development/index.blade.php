...
<div class="container-fluid mt-4 table-container">
    <div style="display: flex; justify-content: space-between; align-items: center;">
        ...
        <div class="search-container">
            <input type="text" class="search-input" placeholder="Search Web Development...">
            <a href="{{ url('web_development/add') }}" class="add-new-btn">Add New </a>
        </div>
    </div>
    ...
    <div class="table-wrapper">
        <table class="table table-bordered table-striped">
            ...
            <tbody>
                @foreach ($webDevelopments as $item)
                <tr>
                    <td>{{ $item->id }}</td>
                    <td>{{ $item->title }}</td>
                    ...
                    <td>
                        @if($item->img)
                            <img src="{{ asset($item->img) }}" width="30px" height="30px" alt="Profile Image">
                        @else
                            No Image
                        @endif
                    </td>
                    <td class="actions">
                        <a href="{{ route('Dashboard.admin.web_development.edit', $item->id) }}"><i class="fas fa-edit" title="Edit"></i></a>
                        <form action="{{ route('Dashboard.admin.web_development.delete', $item->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <button type="submit" style="border: none; background: none; cursor: pointer;padding:0px;">

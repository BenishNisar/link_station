...
<div class="container-fluid mt-4 table-container">
    <div style="display: flex; justify-content: space-between; align-items: center;">
        <h1 style="font-size: 23px; font-weight: bolder;">Digital Marketing</h1>

        <div class="search-container">
            <input type="text" class="search-input" placeholder="Search digital...">
            <a href="{{ url('digital_market/add') }}" class="add-new-btn">Add New </a>
        </div>
    </div>
    ...
    <div class="table-wrapper">
        <table class="table table-bordered table-striped">
            ...
            <tbody>
                @foreach ($digitalMarketingRecords as $item)
                <tr>
                    <td>{{ $item->id }}</td>
                    <td>{{ $item->title }}</td>
                    ...
                    <td class="actions">
                        ...
                        <a href="{{ route('Dashboard.admin.digital_market.edit', $item->id) }}"><i class="fas fa-edit" title="Edit"></i></a>
                        <form action="{{ route('Dashboard.admin.digital_market.delete', $item->id) }}" method="POST" style="display: inline;">
                            @csrf
                            @method('DELETE')
